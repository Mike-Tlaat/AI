...
import {
  ...
  getQuestionsByExam,
  submitExamAttempt,
} from "../includes/functions.js?v=3.0.0";
...
function updatePageTitle(exam) {
  ...
  const isShabab = examName.includes("شباب") || exam.slug?.includes("shabab");
...
async function showRegistration() {
  ...
  // تحميل قائمة الكنائس الخاصة بهذا الامتحان (أو كل الكنائس لو مفيش تخصيص)
  ...
  if (churchSelect) {
    try {
      ...
      churchSelect.innerHTML =
        ...
        churches.map((c) => `<option value="${escapeHtml(c.name)}">${escapeHtml(c.name)}</option>`).join("");
    } catch {
...
async function showPackages() {
  ...
  container.innerHTML = categories
    .map((cat) => {
      const limitParts = [];
      if (cat.min_select > 0) limitParts.push(`لازم تختار ${cat.min_select} على الأقل`);
      limitParts.push(cat.max_select ? `بحد أقصى ${cat.max_select}` : "بلا حد أقصى");
    ...
    newReviewBtn.addEventListener("click", () => {
      ...
      container.querySelectorAll(".pkg-card").forEach((card) => {
        ...
        if (min > 0) {
          ...
          if (checkedCount < min && !missingCategory) {
            missingCategory = { name: card.dataset.category, min, count: checkedCount };
          }
        }
      });
      ...
      const summaryHtml = Object.entries(selections)
        ...
        .map(([catId, itemIds]) => {
          ...
          const itemNames = itemIds.map((id) => cat.items.find((it) => it.id === id)?.name).filter(Boolean);
      ...
      showModal({
        ...
        onCancel: () => {},
        onConfirm: async () => {
          await savePackageSelections(currentAttempt.id, currentExam.id, selections);
          await ensureExamStarted(currentAttempt.id);
          currentAttempt = await getAttempt(currentAttempt.id);
          startExam();
        },
...
function collectSelections(container) {
  ...
  container.querySelectorAll(".pkg-card").forEach((card) => {
    ...
    const values = Array.from(card.querySelectorAll("input:checked")).map((i) => Number(i.value));
