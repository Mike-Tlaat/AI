...
// الكنائس المتاحة لامتحان معين فقط (بدون أي fallback).
// لو الامتحان مالوش أي كنيسة متعلقة به، ترجع مصفوفة فاضية = يعني خانة الكنيسة
// متختفي بالكامل من فورم التسجيل لهذا الامتحان.
...
export async function createPackageCategory(
  examId,
  { name, min_select = 0, max_select = null, sort_order = 0 },
) {
  const { data, error } = await supabase
    .from("package_categories")
...
export async function savePackageSelections(attemptId, examId, selections) {
  ...
  for (const cat of categories) {
    ...
    chosenIds = [...new Set(chosenIds.map(Number))].filter((id) =>
      validItemsById.has(id),
    );
...
export async function getPackageSelections(attemptId) {
  ...
  (data || []).forEach((row) => {
    ...
    if (!result[row.category].includes(row.item))
...
export async function adminAddAttemptPackageItem(
  attemptId,
  categoryId,
  itemId,
) {
