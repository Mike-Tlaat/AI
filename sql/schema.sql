...
-- طابور التسليم لمنع ضغط الإرسال المتزامن
create table if not exists submission_queue (
  queue_token text primary key,
  exam_id uuid not null references exams(id) on delete cascade,
  student_name text not null,
  student_phone text not null,
  created_at timestamptz not null default now(),
...
create index if not exists idx_submission_queue_exam on submission_queue(exam_id);
create index if not exists idx_submission_queue_expires on submission_queue(expires_at);

-- بيانات دخول الأدمن (تُحفظ كـ hash داخل قاعدة البيانات)
...
-- بيانات دخول الأدمن (تُحفظ كـ bcrypt hash)
...
create policy "deny_all_admin_credentials"
...
revoke all on function check_admin_password(text) from public;
grant execute on function check_admin_password(text) to anon, authenticated;
...
-- ============================================================
-- ملاحظة أمان مهمة:
-- كلمة مرور الأدمن الآن محفوظة داخل قاعدة البيانات كـ hash،
-- والتحقق يتم عبر RPC `check_admin_password`.
-- لو عايز حماية أقوى في المستقبل ينفع نضيف Supabase Auth.
-- ============================================================
...
drop policy if exists "public_all_submission_queue" on submission_queue;
create policy "public_all_submission_queue" on submission_queue for all using (true) with check (true);
