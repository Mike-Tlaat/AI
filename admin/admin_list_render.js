...
async function getCategoriesForExamCached(examId) {
  if (!packageCategoriesCache.has(examId)) {
    packageCategoriesCache.set(
      examId,
      await getPackageCategoriesForExam(examId),
    );
  }
...
export async function renderAdminAttemptsPage(tab) {
  ...
  const churchOptionsHtml = churches
    .map(
      (c) =>
        `<option value="${escapeHtml(c.name)}" ${filterChurch === c.name ? "selected" : ""}>${escapeHtml(c.name)}</option>`,
    )
    .join("");
        ...
        <div class="a-pkg-tags">
          ${
            items.length
              ? items
                  .map(
                    (it) => `<span class="a-pkg-tag">${escapeHtml(it)}</span>`,
                  )
                  .join("")
  ...
  async function renderPkgEditArea(attemptId, examId) {
          ...
          currentRows.length
            ? currentRows
                .map(
                  (
                    r,
                  ) => `<span class="a-pkg-tag editable">${escapeHtml(r.category)}: ${escapeHtml(r.item)}
    ...
    function fillItems() {
      ...
      itemSelect.innerHTML = (cat?.items || [])
    ...
    document
      .getElementById(`pkgAddBtn_${attemptId}`)
      .addEventListener("click", async () => {
