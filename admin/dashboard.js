import { getAllExams, createExam, updateExamStatus, deleteExam } from "../includes/functions.js";
  ...
  function renderGrid() {
    ...
    grid.querySelectorAll(".copy-link-btn").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        try {
          ...
          setTimeout(() => (btn.innerHTML = `<i class="fa-solid fa-copy"></i> نسخ الرابط`), 1500);
        } catch {
          alert(url);
        }
      });
    ...
    grid.querySelectorAll(".delete-exam-btn").forEach((btn) => {
      btn.addEventListener("click", async () => {
        ...
        if (!confirm(`هل أنت متأكد من حذف امتحان "${name}" نهائياً؟\nسيتم حذف كل الأسئلة والمحاولات المرتبطة به!`)) return;
        try {
          await deleteExam(id);
