...
async function renderChurchesTab() {
  ...
  const [allChurches, linkedIds] = await Promise.all([
    getAllChurches(),
    getExamChurchLinks(examId),
  ]);
  ...
  function renderMasterList() {
    ...
    listEl.innerHTML = allChurches.length
      ? allChurches
          .map(
            (c) =>
              `<div class="a-master-church-item"><span>${esc(c.name)}</span><button type="button" class="a-mini-btn danger remove-church-btn" data-id="${c.id}"><i class="fa-solid fa-trash-can"></i></button></div>`,
          )
          .join("")
  ...
  document
    .getElementById("selectAllChurchesBtn")
    .addEventListener("click", () => {
      document
        .querySelectorAll("#examChurchesCheckboxes input")
  ...
  document
    .getElementById("saveExamChurchesBtn")
    .addEventListener("click", async () => {
      const checked = Array.from(
        document.querySelectorAll("#examChurchesCheckboxes input:checked"),
...
async function renderPackagesTab() {
        ...
        ${
          cat.items
            .map(
              (
                it,
              ) => `<span class="a-pkg-tag editable" data-item-id="${it.id}">${esc(it.name)}
  ...
  listEl.querySelectorAll(".a-category-card").forEach((card) => {
    ...
    card.querySelector(".save-cat-btn").addEventListener("click", async () => {
      ...
      const min_select =
        Number(card.querySelector(".cat-min-input").value) || 0;
