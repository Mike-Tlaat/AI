...
function showScreen(id) {
  ["loadingScreen","lockedScreen","notFoundScreen","alreadyDoneScreen","startScreen","examScreen","resultScreen","submittedHiddenScreen"]
...
async function init() {
  if (!EXAM_ID) { showScreen("notFoundScreen"); return; }
  sessionKey = `exam_session_${EXAM_ID}`;

  const { data: exam, error: examErr } = await supabaseClient.from("exams").select("*").eq("id", EXAM_ID).single();
  if (examErr || !exam) { showScreen("notFoundScreen"); return; }
  examData = exam;

  if (exam.is_locked) {
    document.getElementById("lockedWhatsapp").href = whatsappLink(`أريد فتح امتحان "${exam.title}" من فضلك`);
  ...
  if (qErr) { showScreen("notFoundScreen"); return; }

  questionsData = questions.map(q => ({
    ...q,
    options: (q.question_options || []).sort((a, b) => a.order_index - b.order_index),
    blanks: (q.question_blanks || []).sort((a, b) => a.blank_index - b.blank_index),
  }));

  document.getElementById("goToResultsLink").href = `results.html?exam=${EXAM_ID}`;
  document.getElementById("goToResultsLink2").href = `results.html?exam=${EXAM_ID}`;
  ...
  if (stored) {
    try {
      ...
      const { data: existing } = await supabaseClient.from("attempts").select("id").eq("exam_id", EXAM_ID).eq("student_phone", session.phone).maybeSingle();
      if (existing) {
  ...
  document.getElementById("examMetaStart").textContent = `مدة الامتحان: ${exam.duration_minutes} دقيقة — عدد الأسئلة: ${questionsData.length}`;
...
async function startExam() {
  ...
  if (!phone || phone.length < 6) { errEl.textContent = "من فضلك اكتب رقم هاتف صحيح"; return; }

  const { data: existing, error } = await supabaseClient.from("attempts").select("id").eq("exam_id", EXAM_ID).eq("student_phone", phone).maybeSingle();
  if (error) { errEl.textContent = "تعذر التحقق، حاول مرة أخرى"; return; }
  if (existing) { showAlreadyDone(); return; }
...
function renderQuestions() {
  ...
  host.innerHTML = questionsData.map((q, i) => {
...
function isAnswered(q) {
  ...
  if (q.question_type === "fill") return Array.isArray(val) && val.length === q.blanks.length && val.every(v => (v || "").trim() !== "");
...
function setBlankAnswer(qId, idx, val, totalBlanks) {
  if (!Array.isArray(session.answers[qId])) session.answers[qId] = new Array(totalBlanks).fill("");
...
function refreshCardState(qId) {
  ...
  if (q.question_type !== "fill") {
    card.querySelectorAll(".option-item").forEach((el, idx) => {
      el.classList.toggle("selected", q.options[idx].id === session.answers[qId]);
    });
  }
}
...
async function submitExam(auto) {
  ...
  if (!auto) {
    const unanswered = questionsData.length - questionsData.filter(isAnswered).length;
    if (unanswered > 0) {
      if (!confirm(`لديك ${unanswered} سؤال بدون إجابة. هل تريد تسليم الامتحان الآن؟`)) return;
    }
  }
  ...
  // تفريغ عملية التسليم بشكل عشوائي بسيط لتخفيف الضغط عند تسليم عدد كبير من
  // الطلاب في نفس اللحظة، مع إعادة المحاولة عند فشل الشبكة
  const staggerDelay = Math.floor(Math.random() * 1500);
  await new Promise(r => setTimeout(r, staggerDelay));

  let gradedTotal = 0, maxTotal = 0;
  ...
  const percentage = maxTotal > 0 ? Math.round((gradedTotal / maxTotal) * 10000) / 100 : 0;
  ...
  if (!success.ok) {
    ...
    statusEl.textContent = "تعذر إرسال الامتحان، برجاء التأكد من الاتصال والمحاولة مجددًا";
    btn.disabled = false;
...
async function insertWithRetry(payload, statusEl, attempt = 1) {
  try {
    ...
    await new Promise(r => setTimeout(r, attempt * 1200 + Math.random() * 800));
...
function showResult(percentage, isPassed, score, maxScore) {
  ...
  document.getElementById("resultGrade").textContent = isPassed ? `${gradeLabelText(percentage)} — ناجح` : "غير ناجح";
  document.getElementById("resultDetail").textContent = `درجتك: ${score} من ${maxScore}`;
}
