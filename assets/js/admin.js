...
async function loadExamIntoBuilder(examId) {
  ...
  renderBuilder();
  const builderTab = document.querySelector('.tab-btn[data-tab="builder"]');
...
function updateQText(localId, val) {
  ...
  if (!q) return;
  const previousBlankCount = q.type === "fill" ? q.blankAnswers.length : 0;
  q.text = val;
  if (q.type === "fill") {
    ...
    if (blanksCount !== previousBlankCount) renderBuilder();
  }
}
...
async function saveExam() {
  ...
  try {
    ...
    resetBuilder();
    const examsTab = document.querySelector('.tab-btn[data-tab="exams"]');
    if (examsTab) {
      examsTab.click();
    } else {
...
async function loadExams() {
  ...
  const publicBaseUrl = new URL("../", location.href).href;

  host.innerHTML = data
    .map((exam) => {
      const examLink = `${publicBaseUrl}exam.html?exam=${exam.id}`;
      const resultsLink = `${publicBaseUrl}results.html?exam=${exam.id}`;
      return `
    <div class="exam-link-row" style="flex-direction:column; align-items:stretch;">
      ...
      <div class="flex-between mt-8">
        <div style="display:flex; gap:8px; flex-wrap:wrap;">
          <button class="btn btn-sm btn-primary" onclick="window.location.href='builder.html?edit=${exam.id}'">تعديل الامتحان</button>
...
document.addEventListener("DOMContentLoaded", () => {
  if (hasElement("questionsBuilder")) renderBuilder();
  if (hasElement("examsList")) loadExams();
  const editId = new URLSearchParams(location.search).get("edit");
  if (editId && hasElement("questionsBuilder")) {
    loadExamIntoBuilder(editId);
  }
});
